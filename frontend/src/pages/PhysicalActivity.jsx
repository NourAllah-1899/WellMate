...
import wordDarkLogo from '../assets/word_dark.png'
...
export default function PhysicalActivity() {
  ...
  // ── Download Planning PDF (single-page, language-aware)
  const downloadPlanningPDF = async () => {
    if (!sportProgram) return

    // Preload dark logo (needs light/white background)
    const logoImg = await new Promise((resolve) => {
      const img = new Image()
      img.onload = () => resolve(img)
      img.onerror = () => resolve(null)
      img.src = wordDarkLogo
    })
    ...
    // ══ HEADER — Deep Navy background for the white logo
    doc.setFillColor(15, 23, 42) // Slate 900 (Very dark)
    ...
    if (logoImg) {
      ...
      doc.addImage(logoImg, 'PNG', MARGIN, 9, logoW, logoH)
    } else {
      doc.setFont('helvetica', 'bold')
      doc.setFontSize(20)
      doc.setTextColor(255, 255, 255)
      doc.text('WellMate', MARGIN, 20)
    }

    // Subtitle + date (right-aligned, white)
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(11)
    doc.setTextColor(255, 255, 255)
    doc.text(lbl.subtitle, PW - MARGIN, 16, { align: 'right' })
    ...
    // ── Program title + objective
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(14)
    doc.setTextColor(15, 23, 42)
    doc.text(sportProgram.program_name || 'Programme Sportif', MARGIN, y)
    y += 6
    if (sportProgram.target_objective) {
      doc.setFont('helvetica', 'italic')
      doc.setFontSize(9)
      doc.setTextColor(79, 70, 229)
      doc.text(sportProgram.target_objective, MARGIN, y)
      y += 5
    }
    y += 2

    // ── Stats row (3 cards, compact)
    const stats = [
      { label: lbl.sessions, value: String(sportProgram.weekly_sessions || '—') },
      { label: lbl.duration,  value: `${sportProgram.session_duration_minutes || '—'} min` },
      { label: lbl.level,     value: (sportProgram.difficulty_level || '—').toUpperCase() },
    ]
    const statW = CW / 3
    stats.forEach((s, i) => {
      ...
      doc.setFontSize(11)
      doc.setTextColor(79, 70, 229)
      doc.text(s.value, x + (statW - 2) / 2, y + 6, { align: 'center' })
      doc.setFont('helvetica', 'normal')
      doc.setFontSize(7)
      doc.setTextColor(71, 85, 105)
      doc.text(s.label, x + (statW - 2) / 2, y + 11, { align: 'center' })
    })
    y += 22
    ...
    doc.setFontSize(11)
    doc.setTextColor(15, 23, 42)
    doc.text(lbl.planning, MARGIN + 5, y)
    y += 3
    ...
    if (Array.isArray(sportProgram.exercises) && sportProgram.exercises.length > 0) {
      const COLS = 3
      const GAP = 2
      const cardW = (CW - GAP * (COLS - 1)) / COLS
      const lineH = 4.2
      const maxActs = Math.max(...sportProgram.exercises.map(e =>
        Array.isArray(e.activities) ? Math.min(e.activities.length, 5) : 0
      ))
      const bodyH = Math.max(1, maxActs) * lineH + 4
      const cardH = 7 + bodyH

      sportProgram.exercises.forEach((ex, idx) => {
        const col = idx % COLS
        ...
        // Day header bar
        doc.setFillColor(79, 70, 229)
        doc.roundedRect(cx, cy, cardW, 7, 1.5, 1.5, 'F')
        doc.setFont('helvetica', 'bold')
        doc.setFontSize(8)
        doc.setTextColor(255, 255, 255)
        doc.text(ex.day || '', cx + cardW / 2, cy + 4.5, { align: 'center' })

        // Card body
        doc.setFillColor(248, 250, 252)
        doc.roundedRect(cx, cy + 7, cardW, bodyH, 0, 0, 'F')
        doc.setDrawColor(224, 231, 255)
        doc.setLineWidth(0.2)
        doc.roundedRect(cx, cy + 7, cardW, bodyH, 0, 0, 'S')

        const acts = Array.isArray(ex.activities) ? ex.activities : []
        let ty = cy + 11
        if (acts.length > 0) {
          acts.slice(0, 5).forEach((act) => {
            doc.setFillColor(99, 102, 241)
            doc.circle(cx + 3, ty + 0.8, 0.7, 'F')
            doc.setFont('helvetica', 'normal')
            doc.setFontSize(7)
            doc.setTextColor(51, 65, 85)
            const truncated = doc.splitTextToSize(act, cardW - 7)[0]
            doc.text(truncated, cx + 5.5, ty + 1)
            ty += lineH
          })
        } else {
          doc.setFont('helvetica', 'italic')
          ...
          doc.text(lbl.rest, cx + cardW / 2, ty + 1, { align: 'center' })
        }
      })

      const rows = Math.ceil(sportProgram.exercises.length / COLS)
      y += rows * (cardH + GAP)
    }

    // ── Recommendations
    if (Array.isArray(sportProgram.recommendations) && sportProgram.recommendations.length > 0) {
      y += 6
      doc.setFillColor(79, 70, 229)
      doc.rect(MARGIN, y - 3, 3, 5, 'F')
      doc.setFont('helvetica', 'bold')
      doc.setFontSize(11)
      doc.setTextColor(15, 23, 42)
      doc.text(lbl.recommendations, MARGIN + 5, y)
      y += 3
      doc.setDrawColor(224, 231, 255)
      doc.setLineWidth(0.4)
      doc.line(MARGIN, y, PW - MARGIN, y)
      ...
      const recCols = 2
      const recGap = 4
      const recW = (CW - recGap) / recCols
      sportProgram.recommendations.slice(0, 6).forEach((rec, idx) => {
        const col = idx % recCols
        const row = Math.floor(idx / recCols)
        const rx = MARGIN + col * (recW + recGap)
        const ry = y + row * 7
        doc.setFillColor(99, 102, 241)
        doc.circle(rx + 2, ry + 1, 0.8, 'F')
        doc.setFont('helvetica', 'normal')
        doc.setFontSize(8)
        doc.setTextColor(51, 65, 85)
        const truncated = doc.splitTextToSize(rec, recW - 6)[0]
        doc.text(truncated, rx + 5, ry + 1.8)
      })
    }
    ...
    doc.setFontSize(7)
    doc.setTextColor(148, 163, 184)
    doc.text(`${lbl.footer} • wellmate.app`, MARGIN, 291)
    doc.text('1 / 1', PW - MARGIN, 291, { align: 'right' })
                ...
                {Array.isArray(sportProgram.exercises) && sportProgram.exercises.length > 0 && (
                  <div className="wm-card">
                    <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
                      <h2 className="text-base font-black">{t('programs.schedule')}</h2>
                      <button
                        type="button"
