...
const HealthSettings = ({ currentGoal, currentType, onUpdate }) => {
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute top-full right-0 mt-4 w-[320px] z-50 animate-wm-fade-in">
          <div className="wm-panel shadow-2xl border-indigo-100 dark:border-indigo-900/50 overflow-hidden bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl">
            ...
            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 pt-1">
              {t('health.settings.title')}
            </h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              {/* Goal Selection */}
              <div>
                <label className="block text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase mb-2 tracking-wider">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {goalTypes.map((gt) => (
                    <button
                      ...
                      className={`p-2.5 rounded-xl border-2 transition-all flex flex-col items-center gap-1 group ${
                        type === gt.id 
                    ...
                    >
                      <span className="text-lg group-hover:scale-110 transition-transform">{gt.icon}</span>
              ...
              <div>
                <div className="flex justify-between items-end mb-2">
                  ...
                  <span className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400">kcal / jour</span>
                </div>
                <div className="relative group">
                  <input
                    ...
                    className="wm-input mt-0 text-xl font-black h-12 px-4 bg-slate-50 dark:bg-slate-800/50 border-transparent focus:border-indigo-500 transition-all rounded-2xl"
                    placeholder="2000"
                  />
                </div>
              </div>
              ...
              <div className="flex gap-2 pt-1">
                <button 
                  ...
                  className="px-4 py-3 rounded-xl font-bold text-xs bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                >
                  {t('common.cancel')}
                </button>
                <button 
                  type="submit" 
                  className="flex-1 py-3 rounded-xl font-black text-xs bg-gradient-to-r from-indigo-600 to-violet-600 text-white shadow-lg shadow-indigo-200 dark:shadow-none hover:shadow-indigo-300 transition-all transform active:scale-95"
                >
                  {t('common.save')}
                </button>
              </div>
